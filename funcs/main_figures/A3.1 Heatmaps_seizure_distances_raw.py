...
folder = os.path.basename(__file__).split(".py")[0] # This will be used to specify the name of the file that the output will be stored in the file results
...
def process_file (in_path):
    ...
    if (n_seizures > 5):
        ...
        fig_name = "seizure_time_dist_{}_Allperm.{}".format (id_patient, "pdf" )
        with PdfPages(os.path.join(out_subfolder, fig_name)) as pages:
            for id_perm in range(0, n_permutations):
                min_value = seizures_time_dist['perm{}'.format(id_perm)].min()
                max_value = seizures_time_dist['perm{}'.format(id_perm)].max()
                fig, ax = plt.subplots(figsize=(12,8))

                formatter = tkr.ScalarFormatter(useMathText=True)
                formatter.set_scientific(True)
                formatter.set_powerlimits((-2, 2))
                g = sns.heatmap(seizures_time_dist['perm{}'.format(id_perm)],
                                xticklabels=x_axis_labels, yticklabels=y_axis_labels,
                                rasterized = True, cbar_kws={"format": formatter,
                                                             "ticks": np.arange(min_value, max_value, 0.1)},
                square = True)
                ax.set_xticklabels ( ax.get_xticklabels (), rotation= 360)
                g.set_xlabel ( "Seizure" )
                g.set_ylabel ( "Seizure" )
                g.set_title ( 'Seizure distance perm{}'.format(id_perm) )
        ...
        list_a = [seizures_dist_eucl_all['perm{}'.format(id_perm)]['IMF{}'.format(imf+1)][0][0].ravel() for imf in range(0, n_imfs)]
        ...
        # Heatmap of Seizure Distance matrix
        for id_perm in range(0, n_permutations):
            fig_name = "seizure_dist_eucl_{}_perm{}.{}".format (id_patient, id_perm, format )
            with PdfPages(os.path.join(out_subfolder, fig_name)) as pages:
                for imf in range(0, n_imfs):
                    ...
                    formatter.set_scientific(True)
                    formatter.set_powerlimits((-2, 2))
                    g = sns.heatmap(seizures_dist_eucl_all['perm{}'.format(id_perm)]['IMF{}'.format(imf+1)][0][0]
                                    , xticklabels=x_axis_labels, yticklabels=y_axis_labels,
                                    rasterized = True, cbar_kws={"format": formatter,
                                                                 "ticks": np.arange(min, max, 0.1)},  square = True,vmin=min, vmax=max)
                    ax.set_xticklabels ( ax.get_xticklabels (), rotation= 360)
                    g.set_xlabel ( "Seizure" )
                    g.set_ylabel ( "Seizure" )
        ...
        for id_perm in range(0, n_permutations):
            fig_name = "NoFixedRange_seizure_dist_eucl_{}_perm{}.{}".format (id_patient, id_perm, format )
            with PdfPages(os.path.join(out_subfolder, fig_name)) as pages:
                for imf in range(0, n_imfs):
                    min_value = seizures_dist_eucl_all['perm{}'.format(id_perm)]['IMF{}'.format(imf+1)][0][0].min()
                    max_value = seizures_dist_eucl_all['perm{}'.format(id_perm)]['IMF{}'.format(imf+1)][0][0].max()
                    fig, ax = plt.subplots(figsize=(12,8))
                    formatter = tkr.ScalarFormatter(useMathText=True)
                    formatter.set_scientific(True)
                    formatter.set_powerlimits((-2, 2))
                    g = sns.heatmap(seizures_dist_eucl_all['perm{}'.format(id_perm)]['IMF{}'.format(imf+1)][0][0]
                                    , xticklabels=x_axis_labels, yticklabels=y_axis_labels,
                                    rasterized = True, cbar_kws={"format": formatter,
                                                                 "ticks": np.arange(min, max, 0.1)},  square = True)
                    ax.set_xticklabels ( ax.get_xticklabels (), rotation= 360)
                    g.set_xlabel ( "Seizure" )
                    g.set_ylabel ( "Seizure" )
                    g.set_title ( 'Seizure distance IMF{}'.format(imf+1) )
